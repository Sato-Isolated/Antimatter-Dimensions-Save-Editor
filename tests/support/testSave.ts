import { decodeSaveString } from '../../src/domain/save/transport/codec';
import { SaveType } from '../../src/domain/save/model';
import { AntimatterDimensionsStruct } from '../../src/Struct';
import {
  SaveFieldDefinition,
  SaveFieldKind,
  readFieldValue,
  resolveFieldPath,
  saveEditorFields,
} from '../../src/domain/save/catalog/fields';
...
const determineProgressionStage = (saveData: AntimatterDimensionsStruct, saveType: SaveType): ProgressionStage => {
  const breakInfinityEnabled = saveType === SaveType.Android ? readUnknown(saveData, 'brake') === true : saveData.break === true;
  ...
  if (
    ...
    saveData.celestials ||
    readUnknown(saveData, 'blackHole') ||
    (saveData.reality && readUnknown(saveData.reality, 'glyphs'))
  ) {
    return 'reality';
  }
  ...
  if (
    hasProgressValue(readUnknown(saveData, 'eternityPoints')) ||
    hasProgressValue(readUnknown(saveData, 'eternities')) ||
    (saveData.eternity && Object.keys(saveData.eternity).length > 0) ||
    readUnknown(saveData, 'timestudy') ||
    saveData.dilation ||
    hasProgressValue(readUnknown(saveData, 'timeShards')) ||
    hasProgressValue(readUnknown(saveData, 'totalTickGained'))
  ) {
    return 'eternity';
  }
  ...
  if (
    ...
    hasProgressValue(readUnknown(saveData, 'infinityPoints')) ||
    hasProgressValue(readUnknown(saveData, 'infinities')) ||
    hasProgressValue(readUnknown(saveData, 'infinitiesBanked')) ||
    hasProgressValue(readUnknown(saveData, 'infinityPower')) ||
    saveData.infinityUpgrades?.length > 0 ||
    saveData.replicanti ||
    hasProgressValue(readUnknown(saveData, 'IPMultPurchases')) ||
    hasProgressValue(readUnknown(saveData, 'ipMultUpgrades')) ||
    hasProgressValue(readUnknown(saveData, 'partInfinityPoint'))
  ) {
    return 'infinity';
  }
 ...
 */
const createStageAppropriateReference = (fullReference: unknown, stage: ProgressionStage, saveType: SaveType): UnknownRecord => {
  const stageReference: UnknownRecord = {};
  
  // Helper function to recursively filter properties
  const filterProperties = (source: unknown, target: UnknownRecord, currentPath = ''): void => {
    ...
    for (const [key, value] of Object.entries(source)) {
      ...
      if (isPropertyRelevantForStage(propertyPath, stage, saveType)) {
        if (isUnknownRecord(value)) {
          const nestedTarget: UnknownRecord = {};
          target[key] = nestedTarget;
          filterProperties(value, nestedTarget, propertyPath);
        } else {
          target[key] = value;
        }
      }
...
const shouldPropertyExist = (propertyPath: string, saveData: AntimatterDimensionsStruct, stage: ProgressionStage, saveType: SaveType): boolean => {
  ...
  if (propertyPath.startsWith('records.')) {
    ...
    if (propertyPath.includes('lastTenInfinities') || propertyPath.includes('InfinityTime')) {
            return hasProgressValue(readUnknown(saveData, 'infinities')) ||
              (saveType === SaveType.Android ? readUnknown(saveData, 'brake') === true : saveData.break === true);
    }
    
    if (propertyPath.includes('lastTenEternities') || propertyPath.includes('EternityTime')) {
            return hasProgressValue(readUnknown(saveData, 'eternities')) ||
              hasProgressValue(readUnknown(saveData, 'eternityPoints'));
    }
    
    if (propertyPath.includes('lastTenRealities') || propertyPath.includes('RealityTime')) {
            return hasProgressValue(readUnknown(saveData, 'realities'));
    }
  ...
  if (propertyPath.startsWith('speedrun.')) {
    ...
    const speedrun = readUnknown(saveData, 'speedrun');
    return saveType === SaveType.PC && isUnknownRecord(speedrun) && speedrun.isUnlocked === true;
  }
  ...
  if (propertyPath.startsWith('replicanti.')) {
    ...
    return saveData.replicanti && readUnknown(saveData.replicanti, 'unl') === true;
  }
  
  // Time study properties only exist if eternity is reached
  if (propertyPath.startsWith('timestudy.')) {
    const eternityPoints = readUnknown(saveData, 'eternityPoints');
  ...
  if (propertyPath.startsWith('blackHole') || propertyPath.includes('blackHole')) {
    const blackHole = readUnknown(saveData, 'blackHole');
    return hasProgressValue(readUnknown(saveData, 'realities')) &&
           Boolean(blackHole) && Array.isArray(blackHole);
  }
  ...
  switch (stage) {
    ...
    case 'infinity':
      ...
      if (propertyPath.includes('infinity') && !propertyPath.includes('eternity') && !propertyPath.includes('reality')) {
        return (saveType === SaveType.Android ? readUnknown(saveData, 'brake') === true : saveData.break === true) ||
               hasProgressValue(readUnknown(saveData, 'infinityPoints')) ||
               hasProgressValue(readUnknown(saveData, 'infinities'));
      }
      if (propertyPath.startsWith('replicanti')) {
 ...
 * @returns An object containing the test result and all differences
 */
export const testSaveData = (
  ...
  try {
    // Decrypt the save
    const decryptedData = decodeSaveString(encryptedSave);
    ...
    result.decryptedData = decryptedData.data as unknown as AntimatterDimensionsStruct;
    ...
    const checkProperties = (
      ...
      if (!isUnknownRecord(decrypted) || !isUnknownRecord(reference)) {
        return;
      }
...
export const checkRealitySection = (
  ...
  const partSimulatedReality = saveType === SaveType.Android
    ? readUnknown(decryptedSave.reality, 'partSimulated')
    : readUnknown(decryptedSave, 'partSimulatedReality');
