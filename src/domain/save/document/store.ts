...
import { resolveStructuredEditPath, validateRegisteredFields } from '../catalog/fields';
import { validateDiscoveredSaveFields } from '../catalog/fieldExplorer';
import {
  DocumentPath,
  SaveChangeMeta,
  SaveObject,
...
const buildDocumentValidation = (
  workingData: SaveObject,
  saveType: SaveType,
  baselineData?: SaveObject,
) => validateDocument(workingData, saveType, baselineData, {
  validateBase: validateDecodedSave,
  validateCatalog: validateRegisteredFields,
  validateDiscovered: validateDiscoveredSaveFields,
});
...
export class SaveEditorStore {
  ...
  loadFromEncoded = (rawSaveData: string = this.state.rawSaveData): { success: boolean; errorMessage: string | null } => {
    ...
    const validation = decryptResult.data
      ? buildDocumentValidation(decryptResult.data, decryptResult.saveType)
      : decryptResult.validation;

    if (!decryptResult.data || !validation.success) {
      const errorMessage = validation.issues.find((issue) => issue.severity === 'error')?.message
    ...
    const document: SaveDocumentSnapshot = {
      ...
      originalData,
      workingData,
      validation: buildDocumentValidation(workingData, decryptResult.saveType, originalData),
      revision: 0,
      transportVersion: decryptResult.transportVersion,
      dataVersion: decryptResult.dataVersion,
      shape: decryptResult.shape,
    };
  ...
  replaceWorkingData = (workingData: SaveObject, source: SaveChangeMeta['source'] = 'json'): void => {
    ...
    const document: SaveDocumentSnapshot = {
      ...this.state.document,
      workingData,
      validation: buildDocumentValidation(workingData, this.state.document.sourceType, this.state.document.originalData),
      revision: this.state.document.revision + 1,
    };
  ...
  updateDocumentAtPath = (path: DocumentPath, value: unknown, source: SaveChangeMeta['source'] = 'structured'): void => {
    ...
    const pathResolution = resolveStructuredEditPath(
      this.state.document.workingData,
      path,
      this.state.document.sourceType,
    );
    const workingData = setValueAtPath(this.state.document.workingData, pathResolution.path, value);
    const document: SaveDocumentSnapshot = {
      ...this.state.document,
      workingData,
      validation: buildDocumentValidation(workingData, this.state.document.sourceType, this.state.document.originalData),
  ...
  encodeWorkingData = (): string => {
    ...
    const validation = buildDocumentValidation(
      this.state.document.workingData,
      this.state.document.sourceType,
      this.state.document.originalData,
    );
    ...
    if (!validation.success) {
      this.patch({
        ...
        encodedOutputData: '',
        errorMessage: validation.issues.find((issue) => issue.severity === 'error')?.message
          ?? 'Save data is invalid and cannot be exported',
      });
      return '';
    }
