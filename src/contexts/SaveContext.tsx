    ...
    try {
      ...
      try {
        ...
        if (response.ok) {
          ...
          throw new Error(`Error retrieving save.json file: ${response.status}`);
        }
      } catch (fetchError) {
        ...
        try {
          const saveJsonModule = await import('../../save.json');
          saveJsonContent = JSON.stringify(saveJsonModule.default);
        } catch (importError) {
          setTestResults({
            success: false,
            errors: [`Unable to load save.json file: ${importError}`]
          });
          return;
        }
      }
