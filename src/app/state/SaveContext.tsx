...
import { createSaveEditorStore, SaveEditorStore } from '../../domain/save/document/store';
import { SaveEditorState, SaveObject } from '../../domain/save/model';
import { getValueAtPath } from '../../domain/save/document/path';

// Interface for save context
export interface SaveContextType {
  originalSaveData: SaveObject | null;
  modifiedSaveData: SaveObject | null;
...
interface SaveActions {
  ...
  resetSave: () => void;
  updateSaveData: (path: string, value: unknown) => void;
  replaceSaveData: (data: SaveObject) => void;
}
  ...
  const actions = useMemo<SaveActions>(() => {
    ...
    return {
      ...
      updateSaveData: (path: string, value: unknown) => {
        if (!path) {
          store.replaceWorkingData(value as SaveObject, 'json');
          return;
        }

        store.updateDocumentAtPath(path, value, 'structured');
      },
      replaceSaveData: (data: SaveObject) => store.replaceWorkingData(data, 'json'),
    };
  }, []);
