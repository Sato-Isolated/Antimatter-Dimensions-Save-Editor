...
import { SaveObject, SaveValidationIssue } from '../../../domain/save/model';
import { parseEditorJson, stringifyEditorJson } from '../../../domain/save/transport/editorJsonCodec';
import { hasDraftConflict } from '../../../domain/save/document/draftConflict';
import { useSaveActions, useSaveSelector } from '../../../app/state/SaveContext';
...
const stringifyDocument = (value: unknown): string => {
  try {
    return stringifyEditorJson(value, 2);
  } catch (error) {
    const message = error instanceof Error ? error.message : 'The document contains an unsupported JSON value.';
    return JSON.stringify({ __editorError: message }, null, 2);
  }
};
...
const isSaveObjectRoot = (value: unknown): value is SaveObject => {
...
const JsonEditor: React.FC<JsonEditorProps> = ({ isActive }) => {
  ...
  const syncEditorState = useCallback((nextDraft: string, resetHistory = true, revision?: number) => {
    clearPendingParse();
    draftBaseRevisionRef.current = revision ?? saveDocument?.revision ?? null;
    setDraft(nextDraft);
  ...
  useEffect(() => {
    ...
    syncEditorState(nextDraft, true, saveDocument.revision);
  ...
  const handleReset = () => {
    ...
    draftBaseRevisionRef.current = saveDocument?.revision ?? null;
  ...
  const applyDraft = (force = false) => {
    ...
    const currentRevision = saveDocument?.revision ?? 0;
    const conflict = hasDraftConflict({
      draft,
      baseline: baselineDraft,
      draftRevision: draftBaseRevisionRef.current,
      currentRevision,
    });
    if (conflict && !force) {
      setLiveMessage('The structured editor changed this save. Reload the draft or force apply it.');
      return;
    }
    ...
    replaceSaveData(parseState.parsed as SaveObject);
  ...
  const handleApply = () => applyDraft(false);

  const handleForceApply = () => applyDraft(true);
  ...
  const handleReloadDraft = () => {
    ...
    clearPendingHistoryCommit();
    syncEditorState(stringifyDocument(saveDocument.workingData), true, saveDocument.revision);
    setLiveMessage('Draft reloaded from the structured save.');
  };
  ...
  const hasConflict = hasDraftConflict({
    draft,
    baseline: baselineDraft,
    draftRevision: draftBaseRevisionRef.current,
    currentRevision: saveDocument?.revision ?? 0,
  });
  ...
  const canApply = !isParsing && parseState.isRootObject && hasLocalChanges && !hasConflict;
  ...
  const statusMessage = isParsing
    ...
    ? parseState.errorMessage
      : !parseState.isRootObject
        ...
        : hasConflict
          ? 'Structured changes are newer than this JSON draft. Reload the draft or force apply it.'
        : hasLocalChanges
          ? 'Draft is ready to apply.'
          : '';
  const showStatus = isParsing || isEditorInvalid || hasLocalChanges || hasConflict;
    ...
    <div className="json-editor-shell" aria-label="Full document JSON editor" aria-busy={isParsing}>
      <div className="json-editor-toolbar">
        ...
        <div className="json-editor-actions">
          ...
          {hasConflict && (
            <>
              <button type="button" className="btn secondary" onClick={handleReloadDraft}>
                Reload draft
              </button>
              <button type="button" className="btn danger" onClick={handleForceApply} disabled={isParsing || !parseState.isRootObject}>
                Force apply
              </button>
            </>
          )}
      ...
      {showStatus && (
        <div
          id={editorStatusId}
          className={`json-editor-status-block${isEditorInvalid ? ' is-error' : ''}${hasConflict ? ' is-conflict' : ''}`}
          role={isEditorInvalid || hasConflict ? 'alert' : 'status'}
        >
